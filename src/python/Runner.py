...
_FW_CONFIG_CONTENT = """\
# Auto-generated by `modena simulate` / `modena init`.
# Re-generated if deleted; safe to extend with additional FireWorks settings.
ADD_USER_PACKAGES:
    - modena
...
def _ensure_fw_config() -> None:
    """Write a minimal FW_config.yaml in cwd if none exists.

    This injects ``ADD_USER_PACKAGES: [modena]`` so that remote and HPC worker
    processes launched from the same directory can import model packages without
    any manual configuration.  An existing file is never overwritten.
    """
    p = Path('FW_config.yaml')
    if not p.exists():
        p.write_text(_FW_CONFIG_CONTENT)
        _log.debug('modena: wrote %s', p.resolve())
