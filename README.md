...
Add these to `~/.bashrc` or `~/.profile` to make them permanent.  The
environment script is generated by CMake at install time from the paths this
build actually used, sets `PATH`, `LD_LIBRARY_PATH`, `PYTHONPATH` and
`MODENA_MATLAB_DIR`, and is safe to source repeatedly.  Use
`<prefix>/share/modena/modena-env.sh` for a custom prefix such as
`/opt/modena`.

For running Python code after a default `$HOME` install you can skip it
entirely: the `modena` package is placed in `~/.local/lib/python3.x/site-packages`,
which Python finds by itself, and `libmodena.so` carries `RUNPATH=$ORIGIN` so
it locates its sibling libraries unaided.  The script is what you need for a
custom prefix, inside a virtualenv, or when building an application that links
against `libmodena`.
